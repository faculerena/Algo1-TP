...
testsPersonales = test [
    ...
    " existeSecuenciaDeAmigos 1" ~: (existeSecuenciaDeAmigos redD usuario1 usuario6) ~?= False,  -- no son amigos

    " existeSecuenciaDeAmigos 2" ~: (existeSecuenciaDeAmigos redD usuario7 usuario6) ~?= True, -- son amigos directos
     
    " existeSecuenciaDeAmigos 3" ~: (existeSecuenciaDeAmigos redD usuario1 usuario5) ~?= True, -- son amigos indirectos

    " nombresDeUsuarios 1" ~: (nombresDeUsuarios redB) ~?=  ["Juan", "Natalia", "Pedro", "Natalia"],

    " nombresDeUsuarios 2" ~: (nombresDeUsuarios ([],[],[])) ~?= [], --red vacia

    " amigosDe 1" ~: (amigosDe redB usuario1) ~?= [usuario2], -- 1 solo amigo

    " amigosDe 2" ~: (amigosDe redB usuario2) ~?= [usuario1, usuario3], -- mas de un amigo

    " amigosDe 3" ~: (amigosDe redB usuario5) ~?= [], -- no tiene ammigos
    
    " amigosDe 4" ~: (amigosDe redB usuario4) ~?= [], -- no esta en la red

    " cantidadDeAmigos 1" ~: (cantidadDeAmigos redB usuario5) ~?= 0, -- no tiene amigos 

    " cantidadDeAmigos 2" ~: (cantidadDeAmigos redB usuario4) ~?= 0, -- no esta en la red

    " cantidadDeAmigos 3" ~: (cantidadDeAmigos redC usuario1) ~?= 4, -- normal
